...
execute if entity @p[predicate=atlantis:in_overworld] as @e[tag=atlantis_trader_room] at @s if predicate atlantis:trader_check_success unless entity @e[type=minecraft:wandering_trader,distance=..16] run function atlantis:events/wandering_trader/spawn_trader

schedule function atlantis:events/wandering_trader/clock 3d replace